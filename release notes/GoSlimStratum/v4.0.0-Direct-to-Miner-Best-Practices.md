...
You're running GoSlimStratum for yourself and your own mining devices (Bitaxe, NerdQAxe, Antminer L7/L9, Baikal BK-G28, GPU miners, etc.).
...
## Multi-Algorithm Notes (v4.0.0)

DTM mode works with all supported algorithms — SHA256d, Scrypt, Skein, and Qubit. The settings above apply regardless of algorithm.

### Algorithm-Specific Considerations

**DigiByte (SHA256d, Scrypt, Skein, Qubit)**
- Set the algorithm in the coin configuration page or `"algorithm"` field in config.json
- One DGB node serves all 4 algorithms — create separate coin pools per algorithm with different stratum ports
- All 4 algorithms support DTM with pool fee and revenue share

**Litecoin (Scrypt)**
- Algorithm is automatically set to scrypt — no configuration needed
- Supports both DTM mode and pool mode with payouts
- MWEB block data is handled automatically

**Dogecoin (Scrypt)**
- Algorithm is automatically set to scrypt — no configuration needed
- **DTM mode only** — pool mode payouts are not supported (DOGE Core uses legacy RPC)
- A license or revenue share acceptance is required (no pool mode fallback)
- No SegWit support — blocks are submitted without witness data

### VarDiff Recommendations by Algorithm

| Algorithm | minDiff | maxDiff | Target Time | Notes |
|-----------|---------|---------|-------------|-------|
| SHA256d | 64 | 65,536+ | 15s | Bitaxe, Antminers, NerdQAxe |
| Scrypt | 1 | 100,000+ | 30s | Covers CPU to L7 ASIC range |
| Skein | 1 | 100,000 | 30s | GPU and Baikal BK-G28 |
| Qubit | 1 | 100,000 | 30s | GPU and Baikal BK-G28 |

For Scrypt, an L7 ASIC at 9 GH/s would need ~4M optimal difficulty. Start with maxDiff 100,000 and raise if flood protection keeps triggering.
